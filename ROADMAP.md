...
- Multi-model experiments are deferred intentionally to protect research project scope and delivery stability.
...
- `0022` Model Selection Governance and Test Access Policy
  Status: Implemented in code and verified, including immutable baseline hashing, access logging, drift blocking, and invalidation of override runs for research claims.
...
- `0003` Dataset Leakage and Split Fix
  Status: Deterministic split manifests, explicit label mapping, and robust manifest loading exist, but dataset-level leakage analysis is not yet complete at research-project depth.
...
- `0026` Problem Formulation and Claim Scope
  Status: The framing is documented in tasks, but final research claim boundaries are still being tightened alongside the method plan.

## Deferred (Out of Scope for Final Delivery)

- `0009` Multi-Model Architecture
  Deferred because a single-model research project with strong governance is the current scope boundary, and multi-model work would expand implementation and tuning cost significantly.
- `0012` Model Ensemble Experiment
  Deferred because ensemble gains are secondary to establishing a defensible single-model robustness baseline within the research project timeline.
...
- `0014` Failure Analysis Pipeline
  Deferred because full error-taxonomy automation would broaden tooling scope beyond the minimum research project delivery, even though manual analysis remains important.
...
- `0025` Compute Budget and Fairness Policy
  Deferred because strict compute-governance formalization is useful for publication-grade benchmarking, but not required to stabilize the final research project delivery.
...
- “Deferred” does not mean discarded; it means intentionally postponed to preserve scope control and research project stability.
